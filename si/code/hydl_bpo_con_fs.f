...
C     HISTORY:  14.12.2005  MOD01.01 fug FS components from lf_decl0.inc
C
C     TODO :   14.12.2005 AUFRAEUMEN... 
      ...
      INTEGER   whatbalm                   ! number of balmer line 1=H_a ...MOD01.01 fug 
c      ! BPO NEW VARS AND DATA
...
c      DIMENSION  FINEST(14),FINSWT(14)
       REAL*8  FINEST(14),FINSWT(14)
       REAL*8  FINwn(7),FINfn(7)
...
c      w, wbalmer(k) - cm,  
c      wfine, hlm, dlamor - A
C     WHAT LINE IS IT (ONLY H_a and H_b TREATED)              ! MOD01.01 fug begin
         OPEN(16,file='pfofile_siu.lst')
      whatbalm=0
      IF ((w.GE.6300.0D-8).AND.(w.LE.6700.0D-8)) whatbalm=1
      IF ((w.GE.4700.0D-8).AND.(w.LE.5100.0D-8)) whatbalm=2          ! MOD01.01 fug end
        WRITE(16,*) '  Balmer line=',w,whatbalm
c   ***   for Balmer lines  nbalm=2
      ...
      IF(mlhf.EQ.0) THEN               ! PRECALCULATIONS 
         ...
         DO 102 I = 1,7  ! ALL 7 FS COMPONNETS
            Kbalm = IPOS-1+I
            FINElm1 = STALPH(Kbalm)*1.D7* conver+wbalmer(1)
            FINElm = wfine(I,whatbalm)                               ! MOD01.01 fug
            FINff = STWTAL(Kbalm)/XN2/3.D0* balmer_gf(1)/8.0
            FINgf = gfine(I,whatbalm)                                ! MOD01.01 fug
            write(16,103) FINElm1*1.0d8, FINff
            write(16,103) FINElm, FINgf                              ! MOD01.01 fug
 102     CONTINUE
...
c1001        continue
 ...
 3001          CONTINUE
               chione(n,idd)=phi_lam_tmp * C !* balmer_gf(k)   !MOD01.01 fug
...
c     sum of fine splitting components
            dwave=w - wbalmer(k)
            HTOTAL = 0.0
            DO 3 I = 1,7                                               !MOD01.01 fug begin
               !findl=FINEST(I)*CONVER
               findl = wfine(I,K)*1.0E-8-wbalmer(k)
               !WRITE(*,*) findl,wfine(I,K),wbalmer(k)
               Dll = dabs(dwave-FINdl)
               !phi=ft4(dll,nfhalf,finedw,chin)*FINSWT(I)
               !WRITE(*,*) "delta",finedl,dll
               phi=ft4(dll,nfhalf,finedw,chin)*gfine(I,K)              !MOD01.01 fug end
               HTOTAL = HTOTAL + phi
               !WRITE(*,*) HTOTAL,phi,I
 3          CONTINUE
